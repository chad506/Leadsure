#!/usr/bin/env python3
"""Sep 12 PM run (Saturday) — WEEKEND PM PASS, THE FINALS HOLD.
No Saturday close. The AM pass locked the Sep 11 finals (triple revision caught);
this run re-spot-checked the three REVISED prints against the history pages —
NVDA 218.29, GOOGL 338.50, MSFT 495.63, all unchanged — so caps/fairs/edges stand.
Calibration: reproduce every Sep 12 AM published fair EXACTLY from the finals.
...
# ---- the Sep 11 FINALS (locked Sep 12 AM; re-spot-checked this run — unchanged) ----
...
# ---- calibration: must reproduce every Sep 12 AM published fair EXACTLY (T = 13/77) ----
...
# ---- BTC ladder color: CoinGecko live Sep 12 ~23:1xZ; Coinbase cross-check 77,214.44 ----
S_bake = 78414.25
S = 77389.61            # CoinGecko live (24h range 76,951.75-78,783.85)
S_am = 77278.23         # the Sep 12 AM print
S_pm11 = 77372.48       # the Sep 11 PM print (~24h ago)
sig = 0.0235
from datetime import datetime, timezone
now = datetime.now(timezone.utc)
end = datetime(2027, 1, 1, 5, 0, tzinfo=timezone.utc)
T = (end - now).total_seconds() / 86400
print(f"\nBTC spot {S}: {(S/S_am-1)*100:+.2f}% vs AM print {S_am}; "
      f"{(S/S_pm11-1)*100:+.2f}% vs Sep11PM {S_pm11}; {(S/S_bake-1)*100:+.2f}% vs bake {S_bake}")
