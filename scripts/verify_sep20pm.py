#!/usr/bin/env python3
"""Sep 20 PM run (Sunday night) — the weekend-close pass: the finals lock a
FOURTH consecutive session on the eve of the first post-breakout close, and
bitcoin drifts to the low end of a fifth straight $80k+ print on THREE live
sources.
1. All five Sep 18 finals re-confirmed to the cent a FOURTH consecutive
   ...
   beneath them a FIFTH session; Sep 16 rows verified beneath those; every
   history-page header still 'At close: Sep 18'). No revision anywhere.
2. Trading-day counts unchanged through the weekend (expect T = 8/72).
3. Calibration: reproduce every published fair EXACTLY
   ...
   P(+-5/wk) 60/58/4) — the board rides into Monday's open unchanged, and
   Monday IS the first close since the quad-witching breakout.
4. Lead/cushion re-derived: 8.95%/$442.1B, 16.01%/$681.4B.
5. BTC color: spot $80,827.29 CoinGecko live (-1.10%/24h, mcap $1.6236T,
   vol $23.29B both fresh; its 24h range 80,154.86-81,864.43 carries the
   AM extremes — both extremes sit inside the two windows' overlap, so a
   live rolling window CAN reproduce them; flagged, not discarded) with
   TWO independent live crosses: Kraken $80,517.00 (-0.85%/24h, own range
   80,212.80-81,850.31, ~0.38% under) and Coinbase $80,260.34 (-1.23%/24h,
   24h-ago ref $81,189.61, ~0.70% under) — the first THREE-live-source
   read since the breakout; spread across the three 0.71%, every read
   over $80k: a FIFTH consecutive $80k+ print, -1.08% on the series high,
   +0.65% on the AM print (the headline firms off the morning), -0.49% vs
   the $80k-break print, +3.08% over the Sep 1 bake. Residuals vs the
   stale pre-breakout Sep 15 walk mids re-widen off the AM easing but
   hold under Saturday's series wides (labeled color only).
AUTODATA: Sep 20 22:45Z cron silent (FORTY-SECOND missed feed run); 23:07Z
poke touching fetch/poke pushed (ee9064d9) — answer checked at EOR."""
import numpy as np
...
# ---- trading days: Sunday night, still no close since Sep 18 ----
...
# ---- the Sep 18 finals: FOURTH consecutive to-the-cent confirmation ----
...
for k in NAMES:
    ...
    print(f"  {k}: {px[k]} ({d:+.2f}%)  cap ${caps[k]:.1f}B  [re-confirmed a 4th pass; vol {vol18[k]:,}]")
print("  [NVDA settled volume 189,973,388 HOLDS; Sep 17 finals re-confirmed a 5th session; Sep 16 rows verified beneath]")
...
print("\n=> Fourth consecutive lock-pass: every fair, edge, badge and Best-Bet stance rides into Monday's open —")
print("   and Monday IS the first close since the quad-witching breakout.")

# ---- BTC color (labeled, never a model input) ----
S = 80827.29        # CoinGecko live (-1.10%/24h; mcap $1.6236T and vol $23.29B fresh;
                    #   its 24h range 80,154.86-81,864.43 carries the AM extremes — both
                    #   sit inside the two windows' overlap; flagged, not discarded)
S_kr = 80517.00     # Kraken LIVE cross (-0.85%/24h, own range 80,212.80-81,850.31)
S_cb = 80260.34     # Coinbase LIVE cross (-1.23%/24h, 24h-ago ref $81,189.61)
S_hi = 81706.31     # Sep 19 PM print — the series high
S_am = 80304.64     # Sep 20 AM print
...
print(f"\nBTC spot {S} (CoinGecko live; Kraken {S_kr} {(S_kr/S-1)*100:+.2f}% and Coinbase {S_cb} "
      f"{(S_cb/S-1)*100:+.2f}% — three live sources, spread {(max(S,S_kr,S_cb)/min(S,S_kr,S_cb)-1)*100:.2f}%):")
print(f"  {(S/S_hi-1)*100:+.2f}% on the series-high print; {(S/S_am-1)*100:+.2f}% on the Sep 20 AM print; "
      f"{(S/S_brk-1)*100:+.2f}% vs the $80k-break print; {(S/S_bake-1)*100:+.2f}% vs bake {S_bake}; T {T:.1f}d; "
      f"needed touch {(85000/S-1)*100:+.1f}%")
assert min(S, S_kr, S_cb) > 80000, "the fifth-$80k-print claim needs all three live reads over $80k"
assert S < S_hi, "the below-the-high claim needs a print under the series high"
mids_b = {85000: 68.0, 90000: 48.5, 95000: 34.5, 100000: 23.5, 110000: 12.0,
          60000: 27.5, 55000: 19.5, 50000: 14.0}
prev_am = {85000: -13.1, 90000: -14.7, 95000: -13.5, 100000: -12.2, 110000: -6.6,
           60000: 5.4, 55000: 8.3, 50000: 9.3}   # Sep 20 AM
...
for H, mid in mids_b.items():
    ...
    r = mid - fairb
    tag = ("re-widens" if abs(r) > abs(prev_am[H]) else "eases") + \
          ("" if abs(r) < abs(wides[H]) else " AT/PAST THE WIDE")
    print(f"  {H/1000:.0f}k: fair {fairb:5.1f}  walk-mid {mid:5.1f}  resid {r:+.1f}  ({tag}; AM {prev_am[H]:+.1f}, wide {wides[H]:+.1f})")
