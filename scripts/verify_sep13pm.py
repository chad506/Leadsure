#!/usr/bin/env python3
"""Sep 13 PM run (Sunday) — WEEKEND PM PASS, THE EVE OF THE FIRST POST-MELT-UP CLOSE.
No weekend close. ALL FIVE Sep 11 finals re-spot-checked against the history pages
a FOURTH consecutive session — NVDA 218.29, AAPL 332.27, GOOGL 338.50, MSFT 495.63,
AMZN 256.78, every one unchanged — so caps/fairs/edges go into Monday as published.
Calibration: reproduce every Sep 12 published fair EXACTLY from the finals.
BTC ladder color at the fresh Kraken spot + rolled T (per the Sep 5 PM precedent;
CoinGecko AND CoinMarketCap both served snapshots tonight — source rotation disclosed)."""
import numpy as np
...
# ---- the Sep 11 FINALS (locked Sep 12 AM; ALL FIVE re-spot-checked again this run — unchanged) ----
...
# ---- BTC ladder color: Kraken live Sep 13 ~23:1xZ (CoinGecko + CMC both snapshot-stale
#      tonight: CG re-served the AM print 77,303.56 to the cent, CMC re-served the Sep 12 AM
#      cross-check 77,262.64; Coinbase live cross-check 76,748.06 — ~0.6% under, disclosed) ----
S_bake = 78414.25
S = 77240.00            # Kraken live (-0.16%/24h)
S_cb = 76748.06         # Coinbase cross-check (-0.84%/24h)
S_am = 77303.56         # the Sep 13 AM print
S_pm12 = 77389.61       # the Sep 12 PM print
sig = 0.0235
from datetime import datetime, timezone
now = datetime.now(timezone.utc)
end = datetime(2027, 1, 1, 5, 0, tzinfo=timezone.utc)
T = (end - now).total_seconds() / 86400
print(f"\nBTC spot {S} (Kraken; Coinbase {S_cb}, {(S_cb/S-1)*100:+.2f}% apart): "
      f"{(S/S_am-1)*100:+.2f}% vs AM print {S_am}; {(S/S_pm12-1)*100:+.2f}% vs Sep12PM {S_pm12}; "
      f"{(S/S_bake-1)*100:+.2f}% vs bake {S_bake}")
